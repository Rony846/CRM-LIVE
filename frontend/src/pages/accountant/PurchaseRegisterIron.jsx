...
import {
  ShoppingCart, Plus, FileText, Download, IndianRupee, Package, Loader2, Eye, Upload, X,
...
const rowActionStyle = { display: 'inline-flex', alignItems: 'center', gap: 4, border: `1px solid ${T.iron200}`, background: T.white, color: T.iron700, borderRadius: 6, padding: '5px 8px', cursor: 'pointer' };
...
export default function PurchaseRegister() {
  ...
  const aiDrafts = (purchases || []).filter(p => p.source === 'whatsapp_agent' && p.doc_status !== 'complete');

  const summaryCards = summary ? [
    { label: 'Total Purchases', value: `${summary.count || 0}`, icon: ShoppingCart, tone: T.orange },
    { label: 'Taxable Value', value: formatCurrency(summary.total_taxable), icon: IndianRupee, tone: T.green },
    { label: 'IGST', value: formatCurrency(summary.total_igst), icon: FileText, tone: T.orange },
    { label: 'CGST + SGST', value: formatCurrency((summary.total_cgst || 0) + (summary.total_sgst || 0)), icon: FileText, tone: '#6D4AB0' },
    { label: 'Total Amount', value: formatCurrency(summary.total_amount), icon: IndianRupee, tone: T.blue },
  ] : [];

  const tableHeaders = ['Purchase #', 'Invoice Date', 'Invoice #', 'Firm', 'Supplier', 'GST Type', 'Status', 'Taxable', 'GST', 'Total', ''];

  const headerRight = (
    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
      <button onClick={() => navigate(-1)} data-testid="back-btn" style={btnOutline} title="Back">
        <ArrowLeft size={14} /> Back
      </button>
      <button onClick={handleExport} style={btnOutline}>
        <Download size={14} /> Export CSV
      </button>
      <button onClick={() => setCreateDialogOpen(true)} data-testid="new-purchase-btn" style={btnPrimary}>
  ...
  return (
    <IronShell
      ...
      subtitle="GST TRACKING · INVENTORY UPDATES"
    ...
    >
      {loading ? (
        ...
        <div data-testid="purchase-register" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>

          {/* ── AI-Drafts Banner ─────────────────────────────────── */}
          {aiDrafts.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, background: T.voltageTint, border: `1px solid #EDDFA6`, borderRadius: 8, padding: 14 }}>
              <div style={{ width: 38, height: 38, borderRadius: 8, display: 'grid', placeItems: 'center', background: '#FBF3D9', color: T.voltageText, flexShrink: 0 }}>
                <AlertTriangle size={18} />
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: T.headline, fontWeight: 700, fontSize: 13, color: T.voltageText, marginBottom: 2 }}>
                  {aiDrafts.length} purchase{aiDrafts.length === 1 ? '' : 's'} created by WhatsApp agent — pending your review
                </div>
                <div style={{ fontSize: 12, color: T.iron700 }}>
                  These were auto-extracted from invoice files sent to the AI assistant. Please verify supplier GSTIN, item HSN codes, and totals, then click <strong>Finalize</strong> on each row.
                </div>
              </div>
            </div>
          )}
          ...
          {waDrafts.length > 0 && (
            <IronCard pad={0} style={{ overflow: 'hidden' }}>
              <div style={{ padding: '12px 14px', borderBottom: `1px solid ${T.iron200}`, background: T.greenTint }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: T.headline, fontWeight: 700, fontSize: 13, color: T.green }}>
                  <FileText size={15} /> From WhatsApp — needs review
                  <span style={badgeStyle('ok')}>{waDrafts.length}</span>
                </div>
                <div style={{ fontSize: 11.5, color: T.iron500, marginTop: 4 }}>
                  Supplier invoices the founder sent to MG Brain that couldn't auto-post (unmatched item / unknown firm). Click <strong>Finalize</strong> to open it pre-filled, map any items, and post.
                </div>
              </div>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead><tr style={{ borderBottom: `1px solid ${T.iron200}`, background: T.iron50 }}>
                    {['Supplier', 'Invoice', 'Firm', 'Items', 'Total', 'Read', 'Action'].map((h, i) => (
                  ...
                  <tbody>
                    {waDrafts.map(d => {
                      ...
                      const allMatched = matched === items.length;
                      return (
                        <tr key={d.id} className="iron-row" style={{ borderBottom: `1px solid ${T.iron200}` }}>
                          <td style={{ ...tdCell, fontWeight: 600, color: T.iron900 }}>{d.supplier_name || '—'}</td>
                          <td style={{ ...tdCell, ...mono, fontSize: 11 }}>
                            {d.invoice_number || '—'}<br /><span style={{ color: T.iron400 }}>{d.invoice_date || ''}</span>
                          </td>
                          <td style={tdCell}>
                            {d.firm_name
                              ? <span style={badgeStyle('ok')}>{d.firm_name}</span>
                              : <span style={badgeStyle('warn')}>unmatched</span>}
                          </td>
                          <td style={{ ...tdCell, fontSize: 11.5 }}>
                            <span style={{ color: allMatched ? T.green : T.voltageText, fontWeight: 600 }}>
                              {matched}/{items.length} matched
                            </span>
                            {d.invoice_file && (
                              <a href={`${API.replace('/api', '')}${d.invoice_file}`} target="_blank" rel="noopener noreferrer"
                                style={{ marginLeft: 8, display: 'inline-flex', alignItems: 'center', gap: 3, color: T.orangeDeep, textDecoration: 'none' }}>
                                <FileText size={11} />invoice
                              </a>
                            )}
                          </td>
                          <td style={{ ...tdCell, textAlign: 'right', ...mono, fontWeight: 600 }}>₹{Number(d.grand_total || 0).toLocaleString('en-IN')}</td>
                          <td style={{ ...tdCell, fontSize: 10.5, color: T.iron500 }}>{d.read_by}<br />({d.confidence})</td>
                          <td style={{ ...tdCell, textAlign: 'right' }}>
                            <div style={{ display: 'inline-flex', gap: 6, justifyContent: 'flex-end' }}>
                              <button onClick={() => finalizeWaDraft(d)} data-testid={`finalize-wa-${d.id}`}
                                style={{ ...btnPrimary, background: T.green, padding: '5px 10px', fontSize: 11 }}>
                                <CheckCircle size={12} /> Finalize
                              </button>
                              <button onClick={() => discardWaDraft(d.id)} title="Discard" style={rowActionStyle}>
                                <Trash2 size={13} color={T.orangeDeep} />
                              </button>
                            </div>
                          </td>
                        </tr>
                ...
                return (
                  <IronCard key={s.label} pad={14}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                      ...
                      <div style={{ width: 32, height: 32, borderRadius: 8, display: 'grid', placeItems: 'center', background: T.iron50, border: `1px solid ${T.iron200}` }}>
                        <Icon size={16} color={s.tone} strokeWidth={1.9} />
                      </div>
                    </div>
          ...
          <IronCard pad={14}>
            <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'flex-end' }}>
              <div style={{ width: 200 }}>
                <Caps size={9} color={T.iron400} style={{ display: 'block', marginBottom: 4 }}>Firm</Caps>
                <select value={selectedFirm} onChange={(e) => setSelectedFirm(e.target.value)} style={{ ...inputStyle, width: '100%', cursor: 'pointer' }}>
              ...
              <div>
                <Caps size={9} color={T.iron400} style={{ display: 'block', marginBottom: 4 }}>From Date</Caps>
                <input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} style={inputStyle} />
              </div>
              <div>
                <Caps size={9} color={T.iron400} style={{ display: 'block', marginBottom: 4 }}>To Date</Caps>
                <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} style={inputStyle} />
              </div>
              <button onClick={() => { setFromDate(''); setToDate(''); setSelectedFirm('all'); }} style={btnOutline}>
                Clear Filters
              </button>
            </div>
          </IronCard>
          ...
          <IronCard pad={0} style={{ overflow: 'hidden' }}>
            <div style={{ padding: '12px 14px', borderBottom: `1px solid ${T.iron200}`, background: T.iron50 }}>
              <div style={{ fontFamily: T.headline, fontWeight: 700, fontSize: 13, color: T.iron900 }}>Purchase Register</div>
            </div>
            {purchases.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '60px 0', color: T.iron400 }}>
                <Package size={44} style={{ margin: '0 auto 10px', opacity: 0.4 }} />
                <div style={{ fontFamily: T.headline, fontWeight: 700, fontSize: 13, color: T.iron700 }}>No purchases found</div>
              </div>
            ) : (
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {purchases.map((purchase) => {
                      ...
                      return (
                        <tr key={purchase.id} className="iron-row" style={{ borderBottom: `1px solid ${T.iron200}` }}>
                          <td style={{ ...tdCell, ...mono, fontWeight: 700, color: T.orangeDeep }}>{purchase.purchase_number}</td>
                          <td style={{ ...tdCell, ...mono, color: T.iron500 }}>{purchase.invoice_date}</td>
                          <td style={{ ...tdCell, ...mono, color: T.iron900 }}>{purchase.invoice_number}</td>
                          <td style={{ ...tdCell, color: T.iron900 }}>{purchase.firm_name}</td>
                          <td style={tdCell}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: T.iron900, fontWeight: 600 }}>
                              {purchase.supplier_name}
                              {purchase.source === 'whatsapp_agent' && (
                                <span title="Created by WhatsApp AI agent — please review" style={badgeStyle('violet')}>AI</span>
                              )}
                            </div>
                            {purchase.supplier_gstin && (
                              <div style={{ ...mono, fontSize: 10.5, color: T.iron400, marginTop: 2 }}>{purchase.supplier_gstin}</div>
                            )}
                          </td>
                          <td style={tdCell}>
                            <span style={badgeStyle(purchase.is_inter_state ? 'info' : 'violet')}>
                              {purchase.is_inter_state ? 'IGST' : 'CGST+SGST'}
                            </span>
                          </td>
                          <td style={tdCell}>
                            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                              <span style={badgeStyle(isDraft ? 'warn' : 'ok')}>{purchase.status || 'final'}</span>
                              {purchase.doc_status && purchase.doc_status !== 'complete' && (
                                <span style={badgeStyle(purchase.doc_status === 'pending' ? 'bad' : 'violet')}>{purchase.doc_status}</span>
                              )}
                            </div>
                          </td>
                          <td style={{ ...tdCell, textAlign: 'right', ...mono, color: T.iron500 }}>
                          ...
                          <td style={{ ...tdCell, textAlign: 'right', ...mono, fontWeight: 700, color: T.iron900 }}>
                          ...
                          <td style={{ ...tdCell, textAlign: 'right' }}>
                            <div style={{ display: 'inline-flex', gap: 6, justifyContent: 'flex-end' }}>
                              {purchase.source === 'whatsapp_agent' && purchase.doc_status !== 'complete' && (
                                <button onClick={() => handleFinalize(purchase)} title="Finalize agent draft" style={rowActionStyle}>
                                  <CheckCircle size={13} color={T.green} />
                                </button>
                              )}
                              <button onClick={() => viewPurchase(purchase)} title="View" style={rowActionStyle}>
                                <Eye size={13} color={T.iron700} />
                              </button>
                              {isAdmin && (
                                <button onClick={() => handleOpenEdit(purchase)} title="Edit Purchase (Admin)" style={rowActionStyle}>
                                  <Pencil size={13} color={T.orangeDeep} />
                                </button>
                              )}
                              {canDelete && ['draft', 'pending'].includes(purchase.doc_status) && (
                                <button onClick={() => handleDelete(purchase)} title="Delete draft purchase" style={rowActionStyle}>
                                  <Trash2 size={13} color={T.orangeDeep} />
                                </button>
                              )}
                            </div>
