...
import { T, Caps, IronCard, badgeStyle } from '@/components/iron/IronKit';
...
const iconBtn = { border: `1px solid ${T.iron200}`, background: T.white, color: T.iron700, borderRadius: 6, padding: '7px 9px', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 4 };

const CAT_TONE = { battery: 'ok', inverter: 'bad', stabilizer: 'info', servo: 'violet', solar: 'warn', accessories: 'slate' };
...
export default function ProductDatasheets() {
  ...
  const catBadge = (category) => badgeStyle(CAT_TONE[category] || 'slate');

  const filterTabs = [
    { id: 'all', label: `All (${datasheets.length})`, icon: null },
    { id: 'battery', label: 'Batteries', icon: Battery },
    { id: 'inverter', label: 'Inverters', icon: Zap },
    { id: 'stabilizer', label: 'Stabilizers', icon: Activity },
  ];
  ...
  const headerRight = (
    <div style={{ display: 'flex', gap: 8 }}>
      <button
        ...
        style={{ ...outlineBtn, borderColor: T.orange, color: T.orangeDeep }}
  ...
  return (
    <IronShell
      ...
      subtitle={`${datasheets.length} CATALOGUE PAGES`}
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <IronCard pad={14}>
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Caps size={9} color={T.iron500}>Amazon Account</Caps>
              <Select value={selectedFirm} onValueChange={setSelectedFirm}>
                <SelectTrigger className="w-48 h-9 bg-white border-slate-300 text-slate-900">
            ...
            {selectedFirm && (
              <button
                ...
                style={{ ...outlineBtn, padding: '7px 12px', borderColor: firmCredentials[selectedFirm] ? T.green : T.voltageText, color: firmCredentials[selectedFirm] ? T.green : T.voltageText }}
            ...
            {!firmCredentials[selectedFirm] && selectedFirm && (
              <span style={{ color: T.voltageText, fontSize: 11, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 6, padding: '7px 13px', fontFamily: T.headline,
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <div style={{ position: 'relative' }}>
              <Search className="w-4 h-4" style={{ position: 'absolute', left: 9, top: '50%', transform: 'translateY(-50%)', color: T.iron400 }} />
              <input
            ...
            <div style={{ display: 'flex', border: `1px solid ${T.iron200}`, borderRadius: 6, background: T.white, padding: 3, gap: 2 }}>
              <button
                onClick={() => setViewMode('grid')}
                style={{ padding: 6, borderRadius: 4, border: 'none', cursor: 'pointer', background: viewMode === 'grid' ? T.iron100 : 'transparent' }}
              >
                <LayoutGrid className="w-4 h-4" style={{ color: viewMode === 'grid' ? T.iron900 : T.iron400 }} />
              </button>
              <button
                onClick={() => setViewMode('list')}
                style={{ padding: 6, borderRadius: 4, border: 'none', cursor: 'pointer', background: viewMode === 'list' ? T.iron100 : 'transparent' }}
              >
                <List className="w-4 h-4" style={{ color: viewMode === 'list' ? T.iron900 : T.iron400 }} />
              </button>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '48px 0', color: T.iron400, fontFamily: T.mono, fontSize: 13 }}>Loading datasheets...</div>
        ) : filteredDatasheets.length === 0 ? (
          <IronCard pad={0}>
            <div style={{ padding: '48px 0', textAlign: 'center' }}>
              <FileText className="w-12 h-12" style={{ color: T.iron400, margin: '0 auto 16px' }} />
              <h3 style={{ fontFamily: T.headline, fontWeight: 700, fontSize: 17, color: T.iron900, marginBottom: 6 }}>No datasheets yet</h3>
              <p style={{ color: T.iron500, marginBottom: 16, fontSize: 13 }}>Create your first product datasheet to get started</p>
              <button onClick={() => setShowCreateDialog(true)} style={{ ...primaryBtn, margin: '0 auto' }}>
                <Plus className="w-4 h-4" /> Create Datasheet
              </button>
            </div>
          </IronCard>
        ) : (
          <div style={viewMode === 'grid'
            ...
            {filteredDatasheets.map((ds) => (
              <IronCard key={ds.id} pad={14} style={viewMode === 'list' ? { display: 'flex', alignItems: 'center', gap: 14 } : undefined}>
                {viewMode === 'grid' ? (
                  <>
                    {/* Thumbnail */}
                    <div style={{ height: 128, background: T.iron50, border: `1px solid ${T.iron200}`, borderRadius: 8, marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                      {ds.image_url ? (
                        <img src={ds.image_url} alt={ds.model_name} style={{ height: '100%', objectFit: 'contain' }} />
                      ) : (
                        <div style={{ color: T.iron400 }}>{categoryIcons[ds.category]}</div>
                      )}
                    </div>

                    {/* Info */}
                    <div style={{ marginBottom: 10 }}>
                      <span style={catBadge(ds.category)}>
                        {ds.category.charAt(0).toUpperCase() + ds.category.slice(1)}
                      </span>
                      <h3 style={{ color: T.iron900, fontFamily: T.headline, fontWeight: 700, fontSize: 14, marginTop: 8 }}>{ds.model_name}</h3>
                      {ds.subtitle && <p style={{ color: T.iron500, fontSize: 12 }}>{ds.subtitle}</p>}
                    </div>

                    {/* Actions */}
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 12 }}>
                      <button style={{ ...outlineBtn, flex: 1, justifyContent: 'center' }} onClick={() => openPreview(ds)}>
                      ...
                      >
                        {pushingToAmazon[ds.id] ? (
                          ...
                          <><Send className="w-3 h-3" /> Amazon</>
                        )}
                      </button>
                      <button style={iconBtn} onClick={() => openEditDialog(ds)}>
                        <Pencil className="w-3 h-3" />
                      </button>
                      <button style={iconBtn} onClick={() => copyPublicLink(ds)}>
                        <Copy className="w-3 h-3" />
                      </button>
                      <button style={{ ...iconBtn, borderColor: '#F6D8BA', color: T.orangeDeep }} onClick={() => handleDelete(ds.id)}>
                  ...
                  <>
                    {/* List view */}
                    <div style={{ width: 48, height: 48, borderRadius: 8, background: T.iron50, border: `1px solid ${T.iron200}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: T.iron400, flexShrink: 0 }}>
                      {categoryIcons[ds.category]}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <h3 style={{ color: T.iron900, fontFamily: T.headline, fontWeight: 700, fontSize: 14 }}>{ds.model_name}</h3>
                      <p style={{ color: T.iron500, fontSize: 12 }}>{ds.subtitle || ds.category}</p>
                    </div>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button style={outlineBtn} onClick={() => openPreview(ds)}>
                        <Eye className="w-3 h-3" /> Preview
                      </button>
                      <button
                        onClick={() => handlePushExistingToAmazon(ds)}
