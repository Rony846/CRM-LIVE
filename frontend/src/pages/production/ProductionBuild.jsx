...
export default function ProductionBuild() {
  ...
  // Gaurav (service_agent/technician) → inverter; Angad (supervisor/call_support) → battery; admin → both
  ...
  useEffect(() => { const t = setTimeout(() => (mode === 'battery' ? bmsRef : mbRef).current?.focus(), 150); return () => clearTimeout(t); }, [mode, skuId]);
  ...
  return (
    <IronShell title="Production — build units" subtitle="Scan components → serial is minted, linked & printed" onRefresh={loadSummary}>
      {/* Mode toggle */}
